...
void VulkanCamera::setOrthographicProjection_minusY(float left, float right, float top, float bottom, float near, float far)
{
...
void VulkanCamera::setOrthographicProjection_plusY(float left, float right, float top, float bottom, float near, float far)
{
	projectionMatrix = glm::mat4{ 1.0f }; // identity matrix
...
void VulkanCamera::setPerspectiveProjection_minusY(float verticalFOV, float aspect, float near, float far)
{
...
void VulkanCamera::setPerspectiveProjection_plusY(float verticalFOV, float aspect, float near, float far)
{
	// aspect = width / height, if aspect near to 0, then this cause error
	assert(glm::abs(aspect - std::numeric_limits<float>::epsilon()) > 0.0f);
	// Constant for computation of matrix
	...
	projectionMatrix[0][0] = 1.0f / (aspect * tanHalfVerticalFOV);
	projectionMatrix[1][1] = -1.0f / tanHalfVerticalFOV; // here comes invert
	projectionMatrix[2][2] = far / (far - near);
	projectionMatrix[2][3] = 1.0f;
	projectionMatrix[3][2] = -(far * near) / (far - near);
}
...
// position * rotation.y * rotation.x * rotation.z
